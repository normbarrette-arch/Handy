...
async changeSpokenCommandsEnabled(enabled: boolean) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("change_spoken_commands_enabled", { enabled }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
},
async updateSpokenCommands(commands: SpokenCommand[]) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("update_spoken_commands", { commands }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
},
async resetSpokenCommands() : Promise<Result<SpokenCommand[], string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("reset_spoken_commands") };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
export type SpokenCommand = { phrase: string; replacement: string; enabled: boolean }
